...
type OrderCreatedPaymentRequestKafkaMessagePublisher struct {
	kafkaModule *kafkamodule.KafkaModule
	mapper      *mapper.OrderMessagingDataMapper
	config      *config.OrderServiceConfigData
}

func NewOrderCreatedPaymentRequestKafkaMessagePublisher(
	kafkaModule *kafkamodule.KafkaModule,
	mapper *mapper.OrderMessagingDataMapper,
	config *config.OrderServiceConfigData,
) *OrderCreatedPaymentRequestKafkaMessagePublisher {
	return &OrderCreatedPaymentRequestKafkaMessagePublisher{
...
func (p *OrderCreatedPaymentRequestKafkaMessagePublisher) Publish(event *event.OrderCreatedEvent) error {
